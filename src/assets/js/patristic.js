(function (global, factory) {
  ...
  function Branch(data, children) {
    ...
    Object.assign(this, {
      ...
      value: data.value || 1,
      selected: data.selected || false, //Guangmeng add
      edgeDistance: data.edgeDistance || 0,
      patristicDistance: data.patristicDistance || 0,
      respresenting: 1
    });
  }
   ...
   * @param  {(Branch|String)} descendant A descendant Branch (or `id` string
   * thereof)
   * @return {Number} The sum of the unit distance of all Branches between the Branch on
   * which it is called and `descendant`. Throws an error if `descendant` is not a
   * descendant of this Branch.
   */
  Branch.prototype.unitDepthOf = function (descendant) {
    let distance = 0;
    if (typeof descendant == "string")
      descendant = this.getDescendant(descendant);
    if (typeof descendant == "undefined")
   ...
   * Guangmeng Add
   * 某一分支下任意两点间的单位距离
   * Computes the unit distance between `descendantA` and `descendantB`.
   * @param  {Branch} descendantA The Branch from which you wish to compute
   * distance
   * @param  {Branch} descendantB The Branch to which you wish to compute distance
   * @return {number} The unit distance between the given descendants.
   */
  Branch.prototype.unitDistanceBetween = function (descendantA, descendantB) {
    let mrca = descendantA.getMRCA(descendantB);
    return mrca.unitDepthOf(descendantA) + mrca.unitDepthOf(descendantB);
  };
   ...
   * this method is called.
   */
  Branch.prototype.unitDistanceTo = function (cousin) {
    if (this==cousin){
      return 0
    }
    let mrca = this.getMRCA(cousin); // 节点同目标节点的最近共同祖先
    return mrca.unitDepthOf(this) + mrca.unitDepthOf(cousin);
  };
   ...
   * @return {Branch} The Branch on which it is called.
   */
  Branch.prototype.getSortDistanceTo = function (target) {
    var edgeDistance;
    var patristicDistance;
    if(this.isLeaf()){
      edgeDistance = this.unitDistanceTo(target);
      patristicDistance = this ==target?-1: Number(this.distanceTo(target).toFixed(5))
    }else{
      let newlist = this.children.map(e=>e.getSortDistanceTo(target))
      let eDists = newlist.map(e=>e[0])
      let pDists = newlist.map(e=>e[1])
      edgeDistance = Math.min(...eDists)
      patristicDistance = Math.min(...pDists)
    }
    this.edgeDistance = edgeDistance
    this.patristicDistance = patristicDistance
    return [edgeDistance,patristicDistance]
  };
   ...
   * @return {Branch} The Branch on which it is called.
   */
  Branch.prototype.getSortDepth = function () {
    var sortDepth;
    if(this.isLeaf()){
      sortDepth = this.depth
    }else{
      let newlist = this.children.map(e=>{
        return e.getSortDepth()
      })
      sortDepth = Math.max(...newlist)
    }
  ...
  /**
   * Guangmeng Add
   * 分支内所有点的edgeDistance都设为0
   * Sets the distance values (height and depth) for each Branch
   * @return {Branch} The Branch on which it is called.
   */
  Branch.prototype.initSortDistance = function () {
    this.edgeDistance = 0;
    this.patristicDistance = 0;
    if (this.children.length){
      this.children.map(el=>{
        return el.initSortDistance()
      });
    }
  };
